...
import ie.conor.models.UserPhotoModel
...
fun uploadImageView(app: FitnessApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Fitness", "uploadTask.exception" + error)
        }
    }
...
fun updateAllFitnessx(app: FitnessApp) {
    ...
    fitnessRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    userfitnessRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
        override fun onCancelled(error: DatabaseError) {}
        override fun onDataChange(snapshot: DataSnapshot) {
            snapshot.children.forEach {
...
fun checkExistingPhoto(app: FitnessApp, activity: Activity) {
